...
let rec count_in_semester sm db = match db with
  |[] -> 0
  |x::xs -> if x.semester = sm
      then 1 + count_in_semester sm xs
      else count_in_semester sm xs
          
let student_avg_grade id db = 
  let rec list_avg sum n l =
    match l with
    |[] -> sum /. n
    |(_, float)::xs -> list_avg (sum +. float) (n +. 1.0) xs
...
let course_avg_grade course db = 
  let rec iter_grades (sum, n) l =
    ...
    |[] -> sum, n
    |(c, g)::xs -> if c = course 
        then iter_grades (sum +. g, n +. 1.0) xs
        else iter_grades (sum, n) xs
  in 
  let rec iter_students (sum, n) l =
    match l with 
    |[] -> sum, n
    |x::xs -> iter_students (iter_grades (sum, n) x.grades) xs
  in let sum, n = iter_students (0.0, 0.0) db 
  in if n = 0.0 then 0.0 else sum /. n
